...
//selon l'etat de station, cette fonction renvoie une image "décrivante"
...
//selon si la station est equipée d'un tpe ou pas , cette fonction renvoie une image qui le décrit
...
//retourne le code HTML d'un pop up
function informationBind(type,libelle,nom,adresse,commune,velos,places, etat){
...
//place les marqueurs sur la carte donnée en paramètre
function placerMarqueurs(map){
  let pointsList = [];
  for (let item of document.querySelectorAll("table#tableauVLille>tbody>tr")){


    let geoloc = JSON.parse(item.dataset.geo);
    let libelle = item.dataset.libelle;
    let nom= item.dataset.nom;
    let etat = item.dataset.etat;
    let commune = item.dataset.commune;
    let adresse = item.dataset.adresse;
    let type = item.dataset.type;
    let velos =item.dataset.velos;
    let places = item.dataset.places;

    let image =  VliveImage.getInstance(velos,places);

    let marker = L.marker(geoloc,{icon:image.getLeafletIcon()} )
    marker.addTo(map).bindPopup(informationBind(type,libelle,nom,adresse,commune,velos,places, etat));
...
// gestion de l'item couran
let itemCourant = null;

  //fixe l'item courant dont le pop up doit s'ouvrir et qui doit s'afficher dans la liste
  function setCurrent(item){
      let previous = item.style.backgroundColor;
      if (itemCourant){
        itemCourant.classList.toggle('current');
        itemCourant.style.backgroundColor=previous;
      }
      ...
      itemCourant.classList.toggle('current');
      itemCourant.style.backgroundColor="yellow";
